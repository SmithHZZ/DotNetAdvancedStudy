//------------------------------------------------------------------------------
// <auto-generated>
//     此代码已从模板生成。
//
//     手动更改此文件可能导致应用程序出现意外的行为。
//     如果重新生成代码，将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

namespace _22_EntityFramework_DBFirst
{
    using System;
    using System.Collections.Generic;
    
    public partial class Colp_Check_List
    {
        public string Id { get; set; }
        public string Parent_Id { get; set; }
        public string Parent_Name { get; set; }
        public string Parent_Code { get; set; }
        public string Organization_Id { get; set; }
        public string Organization_Name { get; set; }
        public string Organization_Code { get; set; }
        public string Department_Id { get; set; }
        public string Department_Name { get; set; }
        public string Department_Code { get; set; }
        public string Equipment_Id { get; set; }
        public string Equipment_Name { get; set; }
        public string Equipment_Code { get; set; }
        public string User_Id { get; set; }
        public string User_Name { get; set; }
        public string User_Code { get; set; }
        public string Code { get; set; }
        public Nullable<System.DateTime> Date { get; set; }
        public string Full_Name { get; set; }
        public string Name { get; set; }
        public Nullable<int> Age { get; set; }
        public string Gender { get; set; }
        public string Nationality { get; set; }
        public Nullable<System.DateTime> BirthDate { get; set; }
        public string IdType { get; set; }
        public string IdNumber { get; set; }
        public string Marriage { get; set; }
        public string Occupation { get; set; }
        public string HomeAddress { get; set; }
        public Nullable<System.DateTime> RegisterDate { get; set; }
        public string PatientDirectory { get; set; }
        public string ContactNumber { get; set; }
        public Nullable<System.DateTime> Lastdaymenses { get; set; }
        public string Source { get; set; }
        public string HCG { get; set; }
        public string Condommode { get; set; }
        public Nullable<int> PregnancyNumber { get; set; }
        public Nullable<int> ProduceNumber { get; set; }
        public Nullable<int> AbortionNumber { get; set; }
        public string BloodType { get; set; }
        public Nullable<int> PartnerNumber { get; set; }
        public Nullable<int> Smoke { get; set; }
        public string Remark { get; set; }
        public string IsRecheck { get; set; }
        public string PregnancyState { get; set; }
        public Nullable<int> PregnancyWeeks { get; set; }
        public string MenopauseStatus { get; set; }
        public Nullable<int> ContactBleeding { get; set; }
        public Nullable<int> IrregularBleeding { get; set; }
        public Nullable<int> AbnormalSecretion { get; set; }
        public Nullable<int> ExogenicLesion { get; set; }
        public Nullable<int> Neoplasm { get; set; }
        public Nullable<int> Ulcer { get; set; }
        public Nullable<int> PregnancyBleeding { get; set; }
        public Nullable<int> GestationHighLesions { get; set; }
        public Nullable<int> PhysicalExamination { get; set; }
        public string TreatmentHistory { get; set; }
        public string MedicalHistory { get; set; }
        public string TCTCheckResult { get; set; }
        public Nullable<System.DateTime> TCTCheckDate { get; set; }
        public string TCTCheckResultSource { get; set; }
        public string HPVCheckResult { get; set; }
        public Nullable<System.DateTime> HPVCheckDate { get; set; }
        public string HPVCheckResultSource { get; set; }
        public string HistopathologyDiagnoseResult { get; set; }
        public Nullable<System.DateTime> HistopathologyDiagnoseDate { get; set; }
        public string HistopathologyDiagnoseResultSource { get; set; }
        public string OtherCharacteristic { get; set; }
        public string CheckDirectory { get; set; }
        public string Appointment_Period { get; set; }
        public string Associated_Documents { get; set; }
        public string Category_Code { get; set; }
        public string Category { get; set; }
        public Nullable<short> State_Code { get; set; }
        public string State { get; set; }
        public Nullable<int> Sort_Code { get; set; }
        public Nullable<short> Allow_Edit { get; set; }
        public Nullable<short> Allow_Delete { get; set; }
        public Nullable<short> Is_Visible { get; set; }
        public Nullable<short> Is_Public { get; set; }
        public Nullable<short> Enabled { get; set; }
        public Nullable<short> Deletion_State_Code { get; set; }
        public string Contents { get; set; }
        public string Description { get; set; }
        public string Tag { get; set; }
        public string Encryption_Code { get; set; }
        public string Software_Version { get; set; }
        public string Language_Version { get; set; }
        public string Term_Version { get; set; }
        public System.DateTime Create_On { get; set; }
        public string Create_User_Id { get; set; }
        public string Create_By { get; set; }
        public System.DateTime Modified_On { get; set; }
        public string Modified_User_Id { get; set; }
        public string Modified_By { get; set; }
        public string Patientsubject { get; set; }
        public Nullable<int> CerviexLesion { get; set; }
        public Nullable<int> HPVInfected { get; set; }
        public string BranchType { get; set; }
    }
}
